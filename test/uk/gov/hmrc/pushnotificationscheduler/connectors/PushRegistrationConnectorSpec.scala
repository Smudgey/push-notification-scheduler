...
class PushRegistrationConnectorSpec extends UnitSpec with WithFakeApplication with ServicesConfig with ScalaFutures with CircuitBreakerTest {
  ...
  private trait Setup extends MockitoSugar {
    ...
    doReturn(successful(unregisteredTokens), Nil: _*).when(mockHttp).GET[Seq[RegistrationToken]](matches(s"${connector.serviceUrl}/push/registration"), argThat(containsTuple[String, String]("maxBatchSize", success.toString)))(any[HttpReads[Seq[RegistrationToken]]](), any[HeaderCarrier]())
    doReturn(successful(previouslyFailedTokens), Nil: _*).when(mockHttp).GET[Seq[RegistrationToken]](matches(s"${connector.serviceUrl}/push/registration"), argThat(containsTuple[String, String]("mode", "recover")))(any[HttpReads[Seq[RegistrationToken]]](), any[HeaderCarrier]())
